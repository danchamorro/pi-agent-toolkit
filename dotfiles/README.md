...
See the [root README Extensions section](../README.md#extensions-28) for the
...
| [MariaDB MCP](https://github.com/MariaDB/mcp) | Read-only MariaDB or MySQL access. Runs via `uvx` with a local connection and optional `MCP_READ_ONLY=true`. | `uvx --from iflow-mcp_mariadb-mariadb-server --with asyncmy==0.2.10 mariadb-server` |
